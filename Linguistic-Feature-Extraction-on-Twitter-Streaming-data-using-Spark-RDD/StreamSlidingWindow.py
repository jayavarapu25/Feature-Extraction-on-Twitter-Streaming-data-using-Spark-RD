...
def extractTweetText(tweetJSON):
    print(tweetJSON)
    logging.critical(tweetJSON)
    if (tweetJSON):
        jsonTweetsText =clean_tweet(tweetJSON[:])
...
if __name__ == '__main__':
    ...
    tweetsDStream = ssc.socketTextStream(sys.argv[1], int(sys.argv[2]),storageLevel=StorageLevel.DISK_ONLY)
    tweetRDD = tweetsDStream.map(lambda tweet: extractTweetText(tweet))
    tweetRDD.pprint(5)
